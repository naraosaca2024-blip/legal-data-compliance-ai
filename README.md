...
```python
# 在客户端中输入问题：
用户: "我们公司想要上线一个数据产品，需要满足哪些合规要求？"

系统输出:
根据《数据安全法》和行业指南，数据产品需满足以下合规要求：
1. 数据分类分级：确定产品涉及的数据敏感等级
2. 隐私合规评估：进行 PIA（隐私影响评估）
3. 安全基线检查：确保符合《等保三级》要求
... （更多建议）
```
...
```python
# 调用合规审查工具：
python compliance_tool.py --check "数据处理协议.pdf" --type data_processing
